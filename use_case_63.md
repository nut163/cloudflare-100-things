# Use Case 63: Using Cloudflare Workers for A/B Testing
...
async function handleRequest(request) {
  ...
  if (cookie && cookie.includes(`${NAME}=a`)) {
    ...
    // User hasn't visited before or didn't receive a variant, assign one.
...
In this example, the worker is listening for fetch events. When a request is received, it checks for a cookie that indicates whether this user has been assigned to variant A or B. If they have, it fetches the corresponding variant. If they haven't, it assigns them to a group, fetches the corresponding variant, and sets a cookie to remember their group.