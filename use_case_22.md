# Use Case 22: Using Cloudflare Workers for A/B Testing
...
async function handleRequest(request) {
  ...
  if (cookie && variants.includes(cookie)) {
    // User has a cookie, return that variant
    url = cookie
  } else {
    // User has no cookie, determine variant and set cookie
