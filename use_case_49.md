...
In this example, the worker checks if the incoming request has a cookie that indicates which variant to serve. If the cookie doesn't exist, it makes a 50/50 decision, fetches the appropriate variant, and sets a cookie for future requests.