# Use Case 83: Using Cloudflare Workers for A/B Testing
...
In this example, the worker is set up to respond to requests with one of two variants, A or B. It uses cookies to persist a visitor's variant, so they see the same variant when they return to the site. The variants are chosen randomly for first-time visitors, with a 50/50 split.